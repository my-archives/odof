...
Events.prototype.trigger = function (events) {
  ...
  for (; i; (j === 0) && (events.length = --i) && (list.length = t = 0)) {
    // concat cache[event] & all
    if (t === 0) {
      event = events[i - 1];
      if (cache[event]) {
        all && (args[0] = event) && (list = all.concat(list));
        // record all start inserted
        (s = list.length) && (list = list.concat(cache[event])) && (t = list.length);
      }
    }
    ...
    if (t) {
      j = list.length;
      if (j--) {
        list[j - 1].apply(list[j] || this, (j > s && rest || args));
        list.length = t = --j;
      }
    }
  }
